...
async def dashboard_overview(
    ...
    try:
        ...
        logger.exception("Dashboard overview fallback activated due to data source error.")
        return DashboardOverview(
            kpis=[
                KpiTile(label="Posts Collected", value="0", delta="Live"),
                KpiTile(label="Pain Signals", value="0", delta="Analyzed"),
                KpiTile(label="Problem Clusters", value="0", delta="Grouped"),
                KpiTile(label="Avg Validation", value="0.0", delta="/100"),
            ],
            top_clusters=[],
            trending_signals=[],
            top_ideas=[],
            ...
            quick_launch_plan=QuickLaunchPlan(
                ...
                bullet_points=[
                    "Dashboard is online, but backend data source is temporarily unavailable.",
                    "Verify database credentials and redeploy backend.",
                ],
            ),
        )
